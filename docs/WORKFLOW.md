...
Экспорт дополнительно формирует `commerce.json`; для формата ВК — `vk.json` с выбранными
товарами, `vk_product_id`, порядком фото и предложенными UTM-ссылками. Существующая UTM-разметка
ссылки не заменяется. Это метаданные для ручной/авторизованной публикации, сетевой отправки нет. Нет скрытой очереди фоновых задач.
Состояние выводится из этих файлов и их хешей.
...
числовые характеристики с единицами и evidence-sensitive термины из `docs/domain.json`.
Например, фраза «исинская глина» должна иметь связанный claim именно об исинской глине,
а не любой подтверждённый факт. **Полную семантическую истинность текста валидатор всё равно
не доказывает** — для этого остаётся factcheck.
...
контента, 2 — некорректная команда/данные/операционная ошибка. Утвердительное лечебное/
псевдонаучное обещание является ошибкой; явное опровержение такого мифа остаётся предупреждением
для ручной проверки контекста.
